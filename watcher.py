...
async def onDelete(message, kelutralBot):
    ...
    embed.set_footer(text="ID: " + str(message.id) + " • Today at " + datetime.now().strftime('%H:%M PST'))
    await channel.send(embed=embed)
...
async def onUpdate(before, after, kelutralBot):
    ...
    if len(removedRoles) > 0:
        ...
        embed.set_footer(text="ID: " + str(after.id) + " • Today at " + datetime.now().strftime('%H:%M PST'))
...
async def onBan(guild, user, kelutralBot):
    ...
    embed.set_footer(text="ID: " + str(user.id) + " • Today at " + datetime.now().strftime('%H:%M PST'))
