...
async def roleUpdate(user):
    ...
    try:
        ...
        for role in activeRoles:
            if role in config.prIDs:
                profile['pr_rank'] = {"id" : role}
                current_rank = get(activeRoles, id=role)
                break
            else:
                current_rank = get(activeRoles, id=782982345523331103)
                await user.add_roles(current_rank)
    try:
        next_rank_index = config.prIDs.index(current_rank.id) - 1
    except AttributeError:
        next_rank_index = config.prIDs.index(782982345523331103)
    next_rank = get(activeRoles, id=config.prIDs[next_rank_index])
    
    now = datetime.strftime(datetime.now(), '%H:%M')
    if message_count >= config.prThresholds[next_rank_index]:
        await user.add_roles(next_rank)
        if current_rank.id != config.prFrapo:
        ...
        embed.add_field(name="New Rank Achieved on Pandora Rising", value="**Congratulations!** By participating in the community, you've leveled up! You're now a {}.".format(next_rank.name), inline=False)
